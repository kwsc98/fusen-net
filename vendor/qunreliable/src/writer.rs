...
impl RawDatagramWriter {
    fn new(tx_wakers: ArcSendWakers) -> Self {
        Self {
            datagrams: VecDeque::with_capacity(DATAGRAM_QUEUE_CAPACITY),
            tx_wakers,
        }
    }
}
...
impl DatagramOutgoing {
    ...
    pub fn new_writer(&self, max_datagram_frame_size: u64) -> io::Result<DatagramWriter> {
        ...
        Ok(DatagramWriter {
            writer: self.0.clone(),
            max_datagram_frame_size: match usize::try_from(max_datagram_frame_size) {
                Ok(value) => value,
                Err(_) => usize::MAX,
            },
        })
    ...
    fn try_read_datagram(&self, mut buf: &mut [u8]) -> Option<(DatagramFrame, usize)> {
        ...
        let data = writer.datagrams.front()?.clone();
        let data_len = VarInt::try_from(data.len()).ok()?;
        ...
        (DatagramFrame, Bytes): Package<P>,
    {
        ...
        let data = match writer.datagrams.front() {
            Some(data) => data.clone(),
            None => return Err(Signals::TRANSPORT),
        };
        let data_len = match VarInt::try_from(data.len()) {
            Ok(data_len) => data_len,
            Err(_) => {
                tracing::error!("dropping Datagram whose length cannot be encoded as QUIC varint");
                writer.datagrams.pop_front();
                return Err(Signals::TRANSPORT);
            }
        };
        let frame_without_len = DatagramFrame::new(false, data_len);
        let frame_with_len = DatagramFrame::new(true, data_len);
        let result = match max_encoding_size {
            // Encode length
            n if n >= frame_with_len.encoding_size() => (frame_with_len, data).dump(packet),
            // Do not encode length, may need padding
            n => {
                packet.put_bytes(0, n - frame_without_len.encoding_size());
                (frame_without_len, data).dump(packet)
            }
        };
        if result.is_ok() {
            writer.datagrams.pop_front();
    ...
    pub fn on_conn_error(&self, error: &Error) {
        let writer = &mut lock_unpoisoned(&self.0);
        if writer.is_ok() {
...
impl DatagramWriter {
    ...
    /// If the bounded send queue is full, it returns [`io::ErrorKind::WouldBlock`].
    pub fn send_bytes(&self, data: Bytes) -> io::Result<()> {
        match &mut *lock_unpoisoned(&self.writer) {
            Ok(writer) => {
                // Only consider the smallest encoding method: 1 byte
                if data.len().saturating_add(1) > self.max_datagram_frame_size
                    || VarInt::try_from(data.len()).is_err()
                {
                ...
                if writer.datagrams.len() >= DATAGRAM_QUEUE_CAPACITY {
                    return Err(io::Error::new(
                        io::ErrorKind::WouldBlock,
                        "Datagram send queue is full",
                    ));
                }
                writer.datagrams.push_back(data);
...
mod tests {
    ...
    #[test]
    fn test_datagram_writer_rejects_new_data_at_queue_capacity() {
        let outgoing = DatagramOutgoing::new(Default::default());
        let writer = outgoing.new_writer(1024).unwrap();

        for _ in 0..DATAGRAM_QUEUE_CAPACITY {
            writer.send_bytes(Bytes::from_static(b"queued")).unwrap();
        }
        let error = writer
            .send_bytes(Bytes::from_static(b"rejected"))
            .expect_err("queue must reject its 257th Datagram");
        ...
        assert!(outgoing.try_read_datagram(&mut buffer).is_some());
        writer
            .send_bytes(Bytes::from_static(b"accepted after drain"))
            .unwrap();
    }
