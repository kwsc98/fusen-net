...
/// Maximum number of application Datagrams buffered in each direction.
pub const DATAGRAM_QUEUE_CAPACITY: usize = 256;

fn lock_unpoisoned<T>(mutex: &Mutex<T>) -> MutexGuard<'_, T> {
    match mutex.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            tracing::warn!("recovering poisoned Datagram state lock");
            poisoned.into_inner()
        }
    }
}
