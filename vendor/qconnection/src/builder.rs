...
use crate::{
    ...
    events::{ArcEventBroker, CONNECTION_EVENT_QUEUE_CAPACITY, EmitEvent, Event},
...
impl PendingConnection {
    ...
    pub fn run(self) -> Connection {
        let (event_broker, events) = mpsc::channel(CONNECTION_EVENT_QUEUE_CAPACITY);
...
fn spawn_drive_connection(mut events: mpsc::Receiver<Event>, state: Arc<ConnectionState>) {
    tokio::spawn(
        async move {
