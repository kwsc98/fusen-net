...
use qbase::{
    ...
    frame::{ConnectionCloseFrame, Frame as V1Frame, GetFrameType, ReceiveFrame, SendFrame},
    net::{
        addr::BindUri,
        route::{Link, Pathway},
        tx::Signals,
...
use crate::{
    ...
    space::{
        AckDataSpace, FRAME_DISPATCH_QUEUE_CAPACITY, FlowControlledDataStreams, Frame,
        assemble_closing_packet, emit_receive_error, filter_odcid_packet, pipe, read_plain_packet,
        try_dispatch_frame,
    },
    state,
...
fn frame_dispathcer(
    ...
    let (ack_frames_entry, rcvd_ack_frames) = mpsc::channel(FRAME_DISPATCH_QUEUE_CAPACITY);
    // 连接级的
    let (max_data_frames_entry, rcvd_max_data_frames) =
        mpsc::channel(FRAME_DISPATCH_QUEUE_CAPACITY);
    let (data_blocked_frames_entry, rcvd_data_blocked_frames) =
        mpsc::channel(FRAME_DISPATCH_QUEUE_CAPACITY);
    let (new_cid_frames_entry, rcvd_new_cid_frames) = mpsc::channel(FRAME_DISPATCH_QUEUE_CAPACITY);
    let (retire_cid_frames_entry, rcvd_retire_cid_frames) =
        mpsc::channel(FRAME_DISPATCH_QUEUE_CAPACITY);
    let (handshake_done_frames_entry, rcvd_handshake_done_frames) =
        mpsc::channel(FRAME_DISPATCH_QUEUE_CAPACITY);
    let (new_token_frames_entry, rcvd_new_token_frames) =
        mpsc::channel(FRAME_DISPATCH_QUEUE_CAPACITY);
    // 数据级的
    let (crypto_frames_entry, rcvd_crypto_frames) = mpsc::channel(FRAME_DISPATCH_QUEUE_CAPACITY);
    let (stream_ctrl_frames_entry, rcvd_stream_ctrl_frames) =
        mpsc::channel(FRAME_DISPATCH_QUEUE_CAPACITY);
    let (stream_frames_entry, rcvd_stream_frames) = mpsc::channel(FRAME_DISPATCH_QUEUE_CAPACITY);
    ...
    let datagram_flow = components.datagram_flow.clone();
    let dispathc_v1_frame = move |frame: V1Frame, _pty: packet::Type, path: &Path| match frame {
        V1Frame::Ack(f) => {
        ...
        V1Frame::NewConnectionId(f) => {
            try_dispatch_frame(
                &new_cid_frames_entry,
                f.frame_type(),
                f,
                "NEW_CONNECTION_ID",
                &event_broker,
        ...
        V1Frame::RetireConnectionId(f) => {
            try_dispatch_frame(
                &retire_cid_frames_entry,
                f.frame_type(),
                f,
        ...
        V1Frame::DataBlocked(f) => {
            try_dispatch_frame(
                &data_blocked_frames_entry,
                f.frame_type(),
                f,
                "DATA_BLOCKED",
                &event_broker,
        ...
        V1Frame::Challenge(f) => {
            if let Err(error) = path.recv_frame(&f) {
                emit_receive_error(&event_broker, error);
            }
        }
        V1Frame::Response(f) => {
            if let Err(error) = path.recv_frame(&f) {
        ...
        #[cfg(feature = "unreliable")]
        V1Frame::Datagram(f, data) => {
            if let Err(error) = datagram_flow.recv_frame(&(f, data)) {
                emit_receive_error(&event_broker, error);
            }
        }
        #[cfg(not(feature = "unreliable"))]
        V1Frame::Datagram(f, _) => {
            super::unexpected_frame(&event_broker, f.frame_type(), "Data");
        }
