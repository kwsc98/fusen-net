...
use qbase::{
    ...
    frame::{
        AckFrame, ConnectionCloseFrame, CryptoFrame, FrameFeature, FrameReader, FrameType,
        GetFrameType, ReceiveFrame, ReliableFrame, StreamCtlFrame, StreamFrame,
    },
    packet::{
...
const FRAME_DISPATCH_QUEUE_CAPACITY: usize = 256;

fn pipe<F: Send + Debug + 'static>(
    mut source: mpsc::Receiver<F>,
    ...
    tokio::spawn(
        async move {
            while let Some(f) = source.recv().await {
                if let Err(error) = destination.recv_frame(&f) {
                    emit_receive_error(&broker, error);
                    break;
                }
            }
        }
...
fn emit_receive_error(broker: &ArcEventBroker, error: Error) {
    match error {
        Error::Quic(error) => broker.emit(Event::Failed(error)),
        Error::App(error) => broker.emit(Event::ApplicationClose(error)),
    }
}

fn try_dispatch_frame<F>(
    ...
    match sender.try_send(frame) {
        ...
        Err(mpsc::error::TrySendError::Full(_)) => {
            broker.emit(Event::Failed(QuicError::new(
                ErrorKind::Internal,
                frame_type.into(),
                format!(
                    "{queue_name} dispatch queue exceeded its {FRAME_DISPATCH_QUEUE_CAPACITY}-frame limit"
                ),
            )));
            false
        }
        Err(mpsc::error::TrySendError::Closed(_)) => {
            broker.emit(Event::Failed(QuicError::new(
                ErrorKind::Internal,
                frame_type.into(),
                format!("{queue_name} dispatch consumer stopped"),
            )));
            false
        }
    }
...
fn unexpected_frame(broker: &ArcEventBroker, frame_type: FrameType, packet_space: &'static str) {
    broker.emit(Event::Failed(QuicError::new(
        ErrorKind::ProtocolViolation,
        frame_type.into(),
        format!("unexpected frame in {packet_space} packet"),
    )));
}
...
mod tests {
    use super::*;
    use crate::{events::CONNECTION_EVENT_QUEUE_CAPACITY, state::ArcConnState};

    #[test]
    fn full_dispatch_queue_reports_connection_error() {
        let (sender, _receiver) = mpsc::channel(1);
        sender.try_send(()).unwrap();

        let (event_sender, mut event_receiver) = mpsc::channel(CONNECTION_EVENT_QUEUE_CAPACITY);
        let broker = ArcEventBroker::new(ArcConnState::new(), event_sender);

        assert!(!try_dispatch_frame(
            &sender,
            FrameType::Ping,
            (),
            "test",
        ...
        let Event::Failed(error) = event_receiver.try_recv().unwrap() else {
            panic!("queue overflow must emit a connection failure");
        };
        assert_eq!(error.kind(), ErrorKind::Internal);
        assert_eq!(error.frame_type(), FrameType::Ping.into());
        assert!(error.reason().contains("256-frame limit"));
    }
}
