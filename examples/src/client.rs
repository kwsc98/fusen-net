...
async fn main() {
    ...
    if let Some(tag) = cli.tag  {
        ...
        tokio::spawn(async move {
            let err
            = client::Client::register(server_host, tag).await;
            info!("{:?}", err);
            drop(send)
        });
    }
    ...
    for item in cli.agent {
        ...
        tokio::spawn(async move {
            ...
            info!("start agent target_tag : {}  target_host : {} , local_port : {}",agent_info[0] , agent_info[1],agent_info[2]);
            let err
            = client::Client::agent(server_host, agent_info[0].to_owned() , agent_info[1].to_owned(),agent_info[2].to_owned()).await;
            info!("{:?}", err);
            drop(send)
        });
    }
