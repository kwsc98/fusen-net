...
pub fn generate(node_id: &str, output: &Path) -> Result<GeneratedToken, TokenError> {
    ...
    let token = format!("stl2_{}", URL_SAFE_NO_PAD.encode(random));
    ...
    Ok(GeneratedToken {
        ...
        enrollment_token_sha256: format!("sha256:{hex}"),
    })
}
...
mod tests {
    ...
    fn generated_token_is_private_valid_and_never_overwritten() {
        ...
        let encoded = token.strip_prefix("stl2_").expect("v2 token prefix");
        assert_eq!(
            URL_SAFE_NO_PAD
                .decode(encoded)
                .expect("valid token encoding")
                .len(),
            32
        );
